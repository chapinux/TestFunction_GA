...
cadre <- 10
# lattice 
x <- seq (-cadre,cadre , length=200)
y <- seq (-cadre,cadre, length=200)
#every x,y couple
points <- expand.grid(x,y)

z <- seq(0,0, length =200)
z <-  0.5 + (sin(sqrt(points$Var1^2 + points$Var2^2))^2 -0.5) / ((1+ 0.001*((points$Var1^2 + points$Var2^2))^2)^2)
z <- z + 0.001*runif(length(z))
